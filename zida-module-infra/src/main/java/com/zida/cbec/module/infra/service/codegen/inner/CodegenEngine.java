package com.zida.cbec.module.infra.service.codegen.inner;
...
import com.zida.cbec.framework.apilog.core.enums.OperateTypeEnum;
...
import com.zida.cbec.framework.common.util.date.LocalDateTimeUtils;
...
import com.zida.cbec.framework.excel.core.annotations.DictFormat;
import com.zida.cbec.framework.excel.core.convert.DictConvert;
...
import com.zida.cbec.module.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 zida-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 zida-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.zida.cbec.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/zida-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/zida-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "zida-module-${table.moduleName}/" + // 顶级模块
                "zida-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "zida-module-${table.moduleName}/" + // 顶级模块
                "zida-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "zida-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "zida-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "zida-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
